...
# Modify DotPlot to set threshold for pct expressed
DotPlot <- function(
    object,
    assay = NULL,
    features,
  ...
  data.plot <- lapply(
    ...
    FUN = function(ident) {
      ...
      pct.exp <- apply(X = data.use, MARGIN = 2, FUN = PercentAbove, threshold = min_count - 1) # mod here (threshold)
...
get.RRHO <- function(results_list,
  ...
  for (dataset_name in names(results_list)) {
    ...
    if (!is.character(rownames(left_df_raw))) {
      left_df_raw = left_df_raw %>% 
        column_to_rownames(var = "gene")
    }
    
    if (!is.character(rownames(right_df_raw))) {
      right_df_raw = right_df_raw %>% 
        column_to_rownames(var = "gene")
    }
